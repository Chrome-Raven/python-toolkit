...
```text
.
├── core_logic/           # Data persistence & validation
│   ├── __init__.py
│   ├── account_manager.py
...
│   ├── __init__.py
│   └── user_input.py
├── my_apps/              # User-facing applications
│   ├── __init__.py
│   └── login_system.py
...
Setup Data: Rename core_logic/user_data.json.example to user_data.json. / 重命名为正式数据文件。

Run: Run main_menu() in my_apps/login_system.py. / 运行主菜单函数。
